...
def generate_css_file(filename, palette):
    ...
    with open(css_filename, "w") as file:
        ...
        for idx, value in enumerate(palette["colors"]):
            ...
            file.write(
                f"  --color{idx}: {rgb_color[0]}, {rgb_color[1]}, {rgb_color[2]};\n"
            )
...
parser = argparse.ArgumentParser(
    description="Generate CSS files from images using Pywal"
)
parser.add_argument(
    "input_filename", help="Path to the input text file containing image filenames"
)
